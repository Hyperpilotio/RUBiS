...
public class UserSession extends Thread
{
  ...
  public UserSession(String threadId, URLGenerator URLGen,
      RUBiSProperties RUBiS, Stats statistics, HttpServletResponse response)
  {
    super(threadId);
  ...
  public void run()
  {
    ...
    long startSession = 0;
    long endSession = 0;
//    System.out.println("into the run method:" + ClientEmulator.isEndOfSimulation());
    ...
    while (!ClientEmulator.isEndOfSimulation())
    {
     ...
     if (debugLevel > 2)
        System.out.println("Thread " + this.getName()
            + ": Starting a new user session for " + username + " and transition..." + transition.isEndOfSession() + 
            "...." + nbOfTransitions+ "...<br>");
          ...
          && !transition.isEndOfSession() && (nbOfTransitions > 0))
      {
        ...
        System.out.println("Thread " + this.getName()
        + ": Starting a new user session for " + username + " and transition..." + transition.isEndOfSession() + 
        "...." + nbOfTransitions+ "..." + lastURL + "<br> ");
        try {
			response.getWriter().println("Thread " + this.getName() + " for " + username + " and transition..." + transition.isEndOfSession() + 
			        "...." + nbOfTransitions+ "..." + lastURL + " responseTime:" + (System.currentTimeMillis() - time) + "ms <br />");
		} catch (IOException e) {
