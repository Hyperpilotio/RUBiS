...
public class RUBiSProperties {
	 ...
	 * rubis.properties file is not found in the classpath, the current thread
	 * is killed.
	 */
	public RUBiSProperties() {
		// Get and check database.properties
		System.out.println(
				"Looking for rubis.properties in classpath (" + System.getProperty("java.class.path", ".") + ")<p>");
		try {
	...
	/**
	 * Creates a new <code>RUBiSProperties</code> instance. If the
	 * filename.properties file is not found in the classpath, the current
	 * thread is killed.
	 *
	 * @param filename
	...
	/**
	 * Returns the value corresponding to a property in the rubis.properties
	 * file.
	 *
	 * @param property
	 *            the property name
	...
	/**
	 * Check for all needed fields in rubis.properties and inialize
	 * corresponding values. This function returns the corresponding
	 * URLGenerator on success.
	 *
	 * @return returns null on any error or the URLGenerator corresponding to
	 *         the configuration if everything was ok.
	 */
	public URLGenerator checkPropertiesFileAndGetURLGenerator(String serverName, Integer port) {
		try {
			...
			BufferedReader categoriesReader = new BufferedReader(
					new FileReader(getProperty("database_categories_file")));
			...
			while (categoriesReader.ready()) {
				...
				System.out.println(nbOfCategories + ":" + line.substring(0, openParenthesis - 1));
			...
			// Create a new URLGenerator according to the version the user has
			// chosen
			System.out.println("\n");
			...
			if (useVersion.compareTo("PHP") == 0)
				...
				System.err.println(
						"Sorry but '" + useVersion + "' is not supported. Only PHP, EJB and Servlets are accepted.");
				return null;
			}
	...
	/**
	 * Get a vector of category names as found in the categories file given in
	 * the categories_file field
	 *
	...
	/**
	 * Get the percentage of unique items given in the
	 * percentage_of_unique_items field
	 *
	 ...
	 *
	 *             Get the maximum number of comments per user given in the
	 *             max_comments_per_user field
	 *
	...
	/**
	 * Get the rsh program path that should be used to run the monitoring
	 * program remotely
	 *
	...
	/**
	 * Get the terminal to use for gnuplot. Usually it is set to 'gif' or
	 * 'jpeg'.
	 *
